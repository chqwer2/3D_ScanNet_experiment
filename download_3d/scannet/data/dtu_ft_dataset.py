...
class DtuFtDataset(BaseDataset):
    ...
    def build_init_metas(self):
        ...
        self.id_list = []
        if self.split != "test":
            with open(f'dtu_configs/dtu_finetune_init_pairs.txt') as f:
        ...
        with open(f'dtu_configs/lists/dtu_test_ground.txt') as f:
            lines = f.readlines()
            for line in lines:
        ...
        if self.opt.full_comb < 0:
            with open(f'nerf_synth_configs/list/lego360_init_pairs.txt') as f:
                for line in f:
